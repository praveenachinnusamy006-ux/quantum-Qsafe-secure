...
async function runSeeder() {
  console.log("⏳ Initializing database seeding workflow...");

  try {
    // 1. Insert complete Vendor Registry Data
    console.log("📦 Migrating vendors payload...");
    await db.execute(sql`
      INSERT INTO vendors (name, category, trust_score, quantum_risk_score, status, certification_level, headquarters, website, description, cert_valid)
      VALUES
    ...
    // 2. Insert Tracking Threat Metrics
    console.log("⚠️ Migrating tracking threat metrics...");
    await db.execute(sql`
      INSERT INTO threats (vendor_id, title, severity, category, description, status, detected_at)
      VALUES
    ...
    console.log("🎉 SUCCESS: Complete workspace seed dataset deployed to database.");
    process.exit(0);
  } catch (error) {
    console.error("❌ Seeding terminated abnormally due to error:", error);
    process.exit(1);
  }
}
